...
#if DEBUG
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(newErrorLogEntry:) name:AVPlayerItemNewErrorLogEntryNotification object:self.player.currentItem];
#endif
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(failToPlayTimeout:) name:AVPlayerItemFailedToPlayToEndTimeNotification object:self.player.currentItem];
...
-(void)layoutSubviews{
    ...
    self.titleLabel.frame = CGRectMake(CGRectGetMaxX(self.backBtn.frame)+5, 0, self.topView.frame.size.width-CGRectGetMaxX(self.backBtn.frame) * 2 - 10, self.topView.frame.size.height);
    if (self.isFullscreen) {
...
- (void)didBecomeActive:(NSNotification *)note {
    if(self.state==WMPlayerStatePlaying && !self.enableBackgroundMode) {
        self.playOrPauseBtn.selected = NO;
        [self.player play];
    }
}
        ...
        if ([keyPath isEqualToString:@"status"]) {
            ...
            switch (status){
                ...
                case AVPlayerItemStatusFailed:{
                    ...
                    ESDLog(@"视频加载失败===%@",error.description);
                    ESDLog(@"[WMPlayer] [ESM3U8] currentItem: %@  videoUrl:%@ ", self.currentItem, self.videoURL);
                }
                    break;
            }
            ...
            if (self.currentItem.playbackBufferEmpty) {
                ESDLog(@"%s WMPlayerStateBuffering",__FUNCTION__);
            ...
            if (self.currentItem.playbackLikelyToKeepUp && self.state == WMPlayerStateBuffering){
                ESDLog(@"55555%s WMPlayerStatePlaying",__FUNCTION__);
                if (self.state==WMPlayerStateStopped||self.state==WMPlayerStatePause) {
...
//        int32_t timeScale = self.player.currentItem.asset.duration.timescale;
        ...
        [self.player seekToTime:CMTimeMakeWithSeconds(seekTime, self.currentItem.currentTime.timescale) completionHandler:^(BOOL finished) {
            self.seekTime = 0;
        }];
    }
}
...
- (void)newErrorLogEntry:(id)notification {
    AVPlayerItemErrorLog *errorLog = self.player.currentItem.errorLog;
    [errorLog.events enumerateObjectsUsingBlock:^(AVPlayerItemErrorLogEvent * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ESDLog(@"[WMPlayer] errorlog event : %@", obj);
    }];
    
}

- (void)failToPlayTimeout:(id)notification {
    ESDLog(@"[WMPlayer] failToPlayTimeout");
