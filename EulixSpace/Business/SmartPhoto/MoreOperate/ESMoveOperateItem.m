...
@interface ESMoveOperateItem () <ESMoveCopyViewDelegate>

@property (nonatomic, strong) ESMoveCopyView *moveCopyView;

@end

@implementation ESMoveOperateItem
                              ...
                              completionHandler:^(ESRspDbAffect *output, NSError *error) {
                                  if (!error) {
                                      if (output.code.intValue == 1022) {
                                        [ESToast toastSuccess:@"移动失败"];
                                          return;
                                      }
                                      
                                      dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//                                          [ESToast toastSuccess:@"移动成功"];
...
- (NSString *)title {
   return NSLocalizedString(@"file_bottom_move", @"移动");
}

- (NSString *)iconName {
   return @"file_bottom_move";
}
