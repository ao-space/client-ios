...
@interface ESVideoAlbumPageVC ()

@end

@implementation ESVideoAlbumPageVC
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    NSArray<ESAlbumModel*> *albums = [ESSmartPhotoDataBaseManager.shared getAlbumDataFromDBByType:ESAlbumTypeVideo];
    if (albums.count > 0 && self.albumModel == nil) {
        self.albumModel = albums[0];
    }
    self.title = NSLocalizedString(@"home_video", @"视频");
    ...
    dispatch_async([ESSmartPhotoAsyncManager shared].requestHandleQueue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self.title = [self.listModule countOfAllPic] > 0 ?
                         [NSString stringWithFormat:@"%@(%lu)", NSLocalizedString(@"home_video", @"视频"), [self.listModule countOfAllPic]] : NSLocalizedString(@"home_video", @"视频");
        });
    });
}
...
- (NSString *)titleForEmpty {
    return NSLocalizedString(@"You dont have any videos yet", @"您还没有任何视频哦~") ;
