...
NSMutableData* BTCDataFromBase58CString(const char* cstring) {
    ...
    __block BIGNUM *bn58 = BN_secure_new();
    BN_set_word(bn58, 58);
    __block BIGNUM *bn = BN_secure_new();
    BN_zero(bn);
    __block BIGNUM *bnChar = BN_secure_new();
...
char* BTCBase58CStringWithData(NSData* data) {
    ...
    BN_CTX* pctx = BN_CTX_new();
    __block BIGNUM *bn58 = BN_secure_new(); BN_set_word(bn58, 58);
    __block BIGNUM *bn0 = BN_secure_new();   BN_zero(bn0);
    __block BIGNUM *bn = BN_secure_new();  BN_zero(bn);
    __block BIGNUM *dv = BN_secure_new();  BN_zero(dv);
    __block BIGNUM *rem = BN_secure_new();  BN_zero(rem);
